...
const listProjects = () => {
  ...
  for (let i = 0; i < ProjectList.length; i++) {
    const projectArr = [];
    projectArr.push(<p key={UUID()} className="set-font"><b>Project Name:</b> {ProjectList[i].projectName}</p>)
    projectArr.push(<p key={UUID()} className="set-font"><b>Project Description:</b> {ProjectList[i].projectDesc}</p>)
    ...
    projectList.push(<div key={UUID()} className="section-divider-center">{projectArr}</div>);
  }
  return projectList;
}
const Project = () => {
